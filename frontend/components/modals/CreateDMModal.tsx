...
export default function CreateDMModal({ isOpen, onClose, onDmCreated }: CreateDMModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dmsAPI.create({ username: username.trim() });
      setUsername('');
      onDmCreated();
      onClose();
    } catch (err: any) {
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-discord-text-header">Create Direct Message</h2>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                <div className="relative">
                  <User className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-discord-text-muted" />
                  <input
                    type="text"
                    value={username}
