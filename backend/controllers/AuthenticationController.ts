import { Request, Response } from "express";
import LoginSchema from "../schemas/LoginSchema";
import {
  comparePassword,
  generateRandomPassword,
  generateSnowflake,
  generateVerificationToken,
  hashPassword,
  signJWT,
} from "../utils/HashUtils";
import RegisterSchema from "../schemas/RegisterSchema";
import { PrismaClient, User } from "@prisma/client";
import {
  sendPasswordResetEmail,
  sendVerificationEmail,
} from "../utils/EmailUtils";
import ForgotPasswordSchema from "../schemas/ForgotPasswordSchema";
import { filterUserData } from "../utils/DataUtils";
import { Verifications } from "../schemas/TypeInterfaces";
import { WEB_DOMAIN } from "../config";
export async function LoginController(req: Request, res: Response) {
  try {
    const data = LoginSchema.safeParse(req.body);
    if (!data.success) {
      res.status(400).json({ error: data.error.issues.map((issue: any) => issue.message).join(', ') });
      return;
    }
    const prisma = (await req.app.get("prisma")) as PrismaClient;
    const user = await prisma.user.findFirst({
      where: {
        email: data.data.email.toLowerCase(),
      },
    });
    if (!user) {
      res.status(404).json({ error: "Email not found" });
      return;
    }
    if (!(await comparePassword(data.data.password, user.password))) {
      res.status(401).json({ error: "Invalid password" });
      return;
    }
    const token = signJWT({
      user_id: user.user_id,
      signed_at: Date.now().toString(),
    });
    res
      .status(200)
      .cookie("auth", token)
      .json({ user: filterUserData(user), token });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
}

export async function RegisterController(req: Request, res: Response) {
  try {
    const data = RegisterSchema.safeParse(req.body);
    if (!data.success) {
      res.status(400).json({ error: data.error.issues.map((issue: any) => issue.message).join(', ') });
      return;
    }
    const prisma = req.app.get("prisma") as PrismaClient;
    const existingEmail = await prisma.user.findFirst({
      where: {
        email: data.data.email.toLowerCase(),
      },
    });
    if (existingEmail) {
      res.status(409).json({ error: "Email already exists" });
      return;
    }
    const existingUsername = await prisma.user.findFirst({
      where: {
        username: data.data.username.toLowerCase(),
      },
    });
    if (existingUsername) {
      res.status(409).json({ error: "Username already exists" });
      return;
    }
    const userData = {
      ...data.data,
      password: await hashPassword(data.data.password),
    };
    const token = generateVerificationToken();
    res.app.get("pendingVerifications").set(token, {
      type: "register",
      data: userData,
    } as Verifications);
    setTimeout(
      () => req.app.get("pendingVerifications").delete(data.data.email),
      1000 * 60 * 60 * 24
    );
    await sendVerificationEmail(data.data.email, token);
    res.status(201).json({ message: "Verification Email Sent." });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
}

export async function VerifyController(req: Request, res: Response) {
  try {
    const token = req.params.token as string;
    const tokenData = req.app.get("pendingVerifications").get(token) as
      | Verifications
      | undefined;
    if (!tokenData) {
      res.status(404).json({ error: "Token not found" });
      return;
    }
    const prisma = req.app.get("prisma") as PrismaClient;
    if (tokenData.type === "register") {
      const existingUser = await prisma.user.findFirst({
        where: {
          email: tokenData.data.email?.toLowerCase(),
        },
      });
      if (existingUser) {
        res.status(403).json({ error: "Email already exists" });
        return;
      }
      await prisma.user.create({
        data: {
          ...(tokenData.data as User),
          user_id: generateSnowflake(),
          nickname: tokenData.data.username,
        },
      });
      req.app.get("pendingVerifications").delete(token);
      res.status(200).redirect(WEB_DOMAIN + "/login?verified=true");
    } else if (tokenData.type === "email") {
      const existingUser = await prisma.user.findFirst({
        where: {
          email: tokenData.data.newEmail.toLowerCase(),
        },
      });
      if (existingUser) {
        res.status(403).json({ error: "Email already exists" });
        return;
      }
      await prisma.user.update({
        where: {
          email: tokenData.data.oldEmail.toLowerCase(),
        },
        data: {
          email: tokenData.data.newEmail.toLowerCase(),
          modified: new Date(),
        },
      });
      req.app.get("pendingVerifications").delete(token);
      req.app.get("userCache").delete(tokenData.data.user_id);
      res.status(200).redirect(WEB_DOMAIN + "/account");
    } else if (tokenData.type === "reset-password") {
      const newPassword = generateRandomPassword();
      const passwordHash = await hashPassword(newPassword);
      await prisma.user.update({
        where: {
          email: tokenData.data.email.toLowerCase(),
        },
        data: {
          password: passwordHash,
          modified: new Date(),
        },
      });
      req.app.get("pendingVerifications").delete(token);
      req.app.get("userCache").delete(tokenData.data.user_id);
      res.redirect(WEB_DOMAIN + `/login?newPassword=${newPassword}`);
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
}

export async function ForgotPasswordController(req: Request, res: Response) {
  try {
    const data = ForgotPasswordSchema.safeParse(req.body);
    if (!data.success) {
      res.status(400).json({ error: data.error.issues.map((issue: any) => issue.message).join(', ') });
      return;
    }
    const prisma = req.app.get("prisma") as PrismaClient;
    const user = await prisma.user.findFirst({
      where: {
        email: data.data.email.toLowerCase(),
      },
    });
    if (!user) {
      res.status(404).json({ error: "Email not found" });
      return;
    }
    const token = generateVerificationToken();
    req.app.get("pendingVerifications").set(token, {
      type: "reset-password",
      email: user.email.toLowerCase(),
      user_id: user.user_id,
    });
    await sendPasswordResetEmail(user.email, token);
    setTimeout(() => {
      req.app.get("pendingVerifications").delete(token);
    }, 1000 * 60 * 60 * 24);
    res.status(200).json({ message: "Password reset email sent." });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
  }
}
